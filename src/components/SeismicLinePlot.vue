  ...
  async processData(myProps: any) {
    if (this.myProps.quickTime && this.myProps.quickTime !== "livePlot") {
      ...
      const data = {
        ...
        let responseData = await this.$http.post(`/bridge/analytics/seismic_data/${localStorage.getItem("cmpnyId")}`, data);
        clearInterval(this.interval);
        this.linedata2.data = [];
        setTimeout(()=>{
    ...
    this.interval = setInterval( async()=>{
            ...
            // this.generateTimeData();
             const data = {
                ...
                "Stat name": this.myProps.stat,
                "Unit": this.myProps.unit,
                "Quick Time Range": "livePlot"
              };
            let responseData = await this.$http.post(`/bridge/analytics/seismic_data/${this.$store.state.companyId}`, data);
            console.log(this.chooseAxis)
            console.log(responseData.data.Trend[0].y);
            // const update = {
            //       x:  [[time.getTime()]],
            //       y: [[Math.random()]]
            //   }
